...
filter="drawtext=text='"
filter+="$1"
filter+=''': expansion=normal: fontfile=opensans.ttf: y=h-line_h-15: x=-(mod(5*n\,w+tw)-tw): fontcolor=white: fontsize=30'

script=`ffmpeg -y -i video.mp4 -vcodec libx264 -pix_fmt yuv420p -vf "drawbox=y=ih-h: color=black@0.4: width=iw: height=60: t=fill, ${filter}" -an final.mp4`
